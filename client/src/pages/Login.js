...
const Login = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem("vb"))
      navigate(
        `../user/${JSON.parse(localStorage.getItem("vb"))["_id"]}/accounts`
      );
  });
  //API
  const login = () => {
    axios
      .post(API_AUTH, data, { withCredentials: true })
      .then((res) => {
      ...
      case "email":
        if (
          ...
          setError({
            ...
            [e.target.name]: {
              status: true,
              message: `${e.target.name} was not available.`,
            },
          });
        } else {
      ...
      case "password":
        ...
        if (e.target.value.length >= 1 && e.target.value.length < 8)
          setError({
            ...
            [e.target.name]: {
              status: true,
              message: `${e.target.name} to be a minimum of 8 characters`,
            },
          });
        else if (
          !!/^(?=.*?[0-9])(?=.*?[A-Z])(?=.*?[#?!@$%^&*\\-_]).{8,}$/.test(
          ...
          setError({
            ...
            [e.target.name]: {
              status: true,
              message: `Include a special character and at least one capital letter`,
            },
          });
        else setError({ ...error, [e.target.name]: { status: false } });
        break;
    }
  };
